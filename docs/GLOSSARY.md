...
**Agent**
An AI model instance executing a task within the Pm.Os platform. Agents are assigned to DAG nodes, have bounded permissions, and emit events to the event log. Not the same as a human user.
...
The task execution model in Pm.Os. Tasks are nodes; dependencies are directed edges. Execution flows from root nodes to leaf nodes. Replaces linear backlogs.
...
**Event**
An immutable record in the append-only event log. Every state change in Pm.Os produces an event. Events are never updated or deleted.
...
The open protocol used by Pm.Os to connect agents to external tools and services. All external integrations use MCP.

**Model**
An AI language model from any provider (Anthropic Claude, OpenAI GPT, Google Gemini, or open-source). Pm.Os is model-agnostic.

**Domain**
A bounded module in the Pm.Os monorepo (agent-orchestration, tool-registry, observability, boards, reporting). Each domain owns its own data and logic.
