...
3. **HIGH — dev-stub gate broke Electron sign-in.** `PMOS_DEV_AUTH=1` was set only on the headless `server` script; Electron `pnpm dev` / packaged app rely on the dev-stub for Phase-1 silent sign-in. **Fixed:** `index.ts` (Electron entry) defaults `PMOS_DEV_AUTH ??= '1'` — trusted desktop context enabled; bare headless still gated.
