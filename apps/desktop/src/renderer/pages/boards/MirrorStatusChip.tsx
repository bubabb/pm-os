  ...
  return (
    ...
    >
      ...
      {status.openConflicts > 0 && (
        <span
          aria-label={`${status.openConflicts} open conflict${status.openConflicts === 1 ? '' : 's'}`}
          className="flex items-center gap-0.5 rounded bg-destructive/10 px-1.5 py-0.5 text-[10px] font-medium text-destructive"
        >
